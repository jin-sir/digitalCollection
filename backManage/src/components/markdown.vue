    ...
    >
      ...
      <el-submenu index="5">
        ...
        <el-menu-item index="5-1"
          ><i class="el-icon-upload2"></i>上传图片</el-menu-item
        >
        <el-menu-item index="5-2"
          ><i class="el-icon-upload"></i>网络图片</el-menu-item
        >
      </el-submenu>
    ...
    <el-dialog title="图片上传" v-model="isUploadShow" :modal="false">
      ...
      >
        ...
        <div class="el-upload__tip" slot="tip">
          请确保后台已经将七牛(server/conf/config.js)相关设置配置完毕
        </div>
      </el-upload>
    </el-dialog>
...
export default {
  ...
  computed: {
    compiledMarkdown() {
      console.log();
      return this.value ? marked(this.value.replace(/<!--more-->/g, "")) : "";
    }
  },
  methods: {
    ...
    beforeUpload(file) {
      ...
      return this.$store
        ...
        .then(response => {
          ...
          if (this.bucketHost === "") {
            this.$notify.error(
              "获取七牛Token失败，请确认您已修改后台服务器的七牛配置文件(server/conf/config.js)"
            );
